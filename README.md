...
Java/Spring Boot golden path - proves the build, scan, sign, push, and GitOps pipeline for JVM services (fawkes#2032)
...
# Or run the JAR
java -jar target/java-fawkes-path-0.1.0-SNAPSHOT.jar
```
...
```bash
docker build -t java-fawkes-path:latest .
```

### Run Container

```bash
docker run -p 8080:8080 java-fawkes-path:latest
```
...
kubectl get deployment java-fawkes-path
kubectl get pods -l app=java-fawkes-path
```
...
```bash
# Sync with ArgoCD
argocd app sync java-fawkes-path

# Check status
argocd app get java-fawkes-path
```

## Monitoring

- **Metrics**: https://grafana.fawkes.idp/d/java-fawkes-path
- **Logs**: https://opensearch.fawkes.idp
- **Traces**: https://grafana.fawkes.idp/explore?ds=tempo
- **Health**: https://java-fawkes-path.fawkes.idp/actuator/health
